...
// ============ Brand Purity Rules (CRITICAL) ============

/**
 * Absolute brand purity rules injected into EVERY prompt.
 * These rules prevent the AI engine from injecting its own branding,
 * contact info, attribution, or any third-party marks into the output.
 * This is a commercial product - zero tolerance for any foreign branding.
 */
export const BRAND_PURITY_RULES = `
## ⛔ 绝对禁止事项（最高优先级 - 违反即作废）
...
- [ ] 全文搜索 "www" → 必须为零结果  
- [ ] 全文搜索 "http" → 必须为零结果
- [ ] 全文搜索 ".com" ".im" ".ai" ".io" ".net" ".org" → 必须为零结果
- [ ] 全文搜索 "powered" "made by" "built by" "created by" → 必须为零结果
- [ ] 全文搜索 "manus" "openai" "claude" "gpt" "gemini" → 必须为零结果
- [ ] 全文搜索 "author" "作者" "制作人" → 必须为零结果
- [ ] 检查每一页页脚 → 只允许页码和用户品牌名称
- [ ] 检查结尾页 → 只有感谢语，无任何联系信息
...
export function buildPPTPrompt(
  ...
  // Reinforce brand purity at the end
  lines.push('## ⛔ 再次强调（交付前必须核验）');
  lines.push('');
  lines.push('**在导出PPTX之前，请逐页检查，确保没有任何邮箱地址、网站URL、"Powered by"、"Made by"、作者署名、AI工具名称或任何第三方标识。结尾页只允许简单感谢语，不得有任何联系信息。违反此规则的交付物将被完全拒绝。**');
  lines.push('');
