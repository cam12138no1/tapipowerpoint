...
export function buildPPTPrompt(
  ...
  lines.push('');
  lines.push('5. **品牌与版权要求**（极其重要，必须严格遵守）：');
  lines.push('   - **绝对禁止**在PPT中出现以下任何内容：');
  lines.push('     * "Powered by XXX"、"由XXX提供支持"或任何类似的技术来源声明');
  lines.push('     * 任何AI系统、模型、平台的名称或品牌标识（如Manus、OpenAI、Claude等）');
  lines.push('     * 任何第三方工具、服务的水印、Logo或广告信息');
  lines.push('     * 任何暗示内容由AI生成的标注或声明');
  lines.push('   - 这是面向大型企业客户的专业交付，必须确保内容看起来完全是人工专业制作');
  lines.push('   - 除非用户明确要求添加特定的品牌信息，否则不得添加任何技术来源标注');
  lines.push('   - 违反此规则将导致交付物被拒绝');
