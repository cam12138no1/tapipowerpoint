...
const taskRouter = router({
  ...
  continue: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        // Append brand purity reminder to every continued interaction
        const brandReminder = '\n\n⛔ 提醒：严禁在PPT任何位置添加邮箱、网址、"Powered by"/"Made by"等署名、AI工具名称或第三方标识。';
        await pptEngine.continueTask(task.engineTaskId, input.userResponse + brandReminder);
  ...
  regenerateSlide: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const regeneratePrompt = `请重新生成第 ${input.slideIndex + 1} 页幻灯片，根据以下要求修改：\n\n${input.instruction}\n\n请保持其他页面不变，只修改指定的页面，并重新生成完整的PPTX文件。\n\n⛔ 重要提醒：严禁在任何页面添加邮箱地址、网站URL、"Powered by"/"Made by"等署名、AI工具名称（Manus/OpenAI/Claude/GPT等）、作者信息或任何第三方标识。结尾页只允许简单感谢语，不得有联系信息。`;
...
function buildDesignInstruction(config: {
  ...
  const lines = [
    // Brand purity rules at the top of every project instruction
    "## ⛔ 绝对禁止事项（最高优先级）",
    "",
    "**严禁在PPT的任何位置出现以下内容，违反即作废：**",
    "- 任何邮箱地址（含@符号的任何内容）",
    "- 任何网站URL/域名（www、http、.com、.im、.ai、.io等）",
    "- 任何署名归属（Powered by、Made by、Built by、Created by、Generated by等）",
    "- 任何AI工具/平台名称（Manus、OpenAI、Claude、GPT、Gemini、ChatGPT等）",
    "- 任何作者/制作人/设计师署名",
    "- 结尾页的联系信息（只允许简单感谢语）",
    "- 任何虚构的联系方式",
    "",
  ...
  lines.push("");
  lines.push("## ⛔ 再次强调：交付前逐页检查，确保无任何邮箱、网址、第三方署名或AI工具名称。");
